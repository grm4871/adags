...
def fulfill_speech(
    ...
    *,
    ...
    """If speech announces an act the JSON left null, fill it."""
    ...
    if not act.get("vote_election") and not re.search(
        ...
        if ev:
            act["vote_election"] = ev.group(1).lower()

    prop = act.get("propose") if isinstance(act.get("propose"), dict) else None
    hollow = prop is None or not (
        str(prop.get("title") or "").strip() or (prop.get("effects") or [])
    )
    added = re.search(
        r"(?:add(?:ing)?|seat(?:ing)?|admit)\s+([a-z][a-z0-9_-]{0,31})\s+as\s+a\s+member\b(?:\s+to\s+(.+))?",
        speech,
        re.I,
    )
    if not added:
        mid_m = re.search(r"add_member\s+([a-z][a-z0-9_-]{0,31})\b", speech, re.I)
        if mid_m:
            val_m = re.search(r"(?:standing\s+)?values?\s*[:\-]\s*(.+)", speech, re.I | re.S)

            class _Add:
                def group(self, i):
                    if i == 1:
                        return mid_m.group(1)
                    return val_m.group(1) if val_m else None

            added = _Add()
    has_add = False
    if prop:
        for fx in prop.get("effects") or []:
            if isinstance(fx, dict) and (
                fx.get("type") == "add_member" or "add_member" in fx
            ):
                has_add = True
    if added and (hollow or not has_add):
        mid = added.group(1).lower()
        values = (added.group(2) or "").strip() or f"You were seated as {mid}."
        bill = {
            "title": f"Admit {mid}",
            "text": speech,
            "effects": [{"type": "add_member", "id": mid, "values": values[:400]}],
        }
        if isinstance(prop, dict):
            if not str(prop.get("title") or "").strip() or str(prop.get("title")).lower() == "untitled":
                prop["title"] = bill["title"]
            if not prop.get("text"):
                prop["text"] = speech
            if not prop.get("effects"):
                prop["effects"] = bill["effects"]
            act["propose"] = prop
        else:
            act["propose"] = bill
    elif hollow and re.search(r"\b(i propose|i move|i file|i introduce|i second)\b", low):
        effects: list[dict[str, Any]] = []
        title = ""
        sp = re.search(
            r"set_param\s+([a-z][a-z0-9_.-]+)\s+to\s+(\S+)",
            speech,
            re.I,
        )
        if sp:
            raw_val = sp.group(2).rstrip(".,;:")
            try:
                value: Any = int(raw_val)
            except ValueError:
                value = raw_val
            effects = [{"type": "set_param", "key": sp.group(1), "value": value}]
            title = f"set_param {sp.group(1)}"
        rg = re.search(r"repeal_goal\s+([a-z0-9_-]+)", speech, re.I)
        if rg and not effects:
            effects = [{"type": "repeal_goal", "id": rg.group(1)}]
            title = f"repeal_goal {rg.group(1)}"
            repl = re.search(r"replace(?:e)? with\s+(?:a\s+)?(.+)", speech, re.I | re.S)
            if repl:
                effects.append(
                    {
                        "type": "set_goal",
        ...
        goal = re.search(
            r"(?:add(?:ing)?|set(?:ting)?|establish(?:ing)?)\s+(?:the\s+)?goal\s*:?\s*(.+)",
            speech,
            re.I | re.S,
        )
        if goal and not effects:
            text = " ".join(goal.group(1).split())
            if president and not act.get("executive"):
                act["executive"] = [{"type": "set_goal", "id": "speech-goal", "text": text[:800]}]
            else:
                title = "Enact a goal"
                act["propose"] = {"title": title, "text": text[:800], "effects": []}
                return act
        if not title:
            title = re.split(r"[.!\n]", speech, maxsplit=1)[0].strip()[:80] or "untitled"
        act["propose"] = {"title": title, "text": speech, "effects": effects}

    if president and not act.get("executive"):
        filed = re.search(
            r"(?:write|file|log|publish|record)\s+(?:to\s+)?(?:the\s+)?(?:workspace\s+)?(?:file\s+)?"
            r"([a-z0-9_./-]{3,40}\.md)?",
            speech,
            re.I,
        )
        goal_bits = re.search(
            r"(?:set|enact|establish|file|declare)\s+(?:the\s+|a\s+|our\s+)?goal\s*:?\s*(.+)",
            speech,
            re.I | re.S,
        )
        exec_fx: list[dict[str, Any]] = []
        if goal_bits:
            exec_fx.append(
                {
                    "type": "set_goal",
                    "id": "speech-goal",
                    "text": " ".join(goal_bits.group(1).split())[:800],
                }
            )
        elif goals_empty and not re.search(
            r"\b(add_member|admit|adding|nominate|join the|vote aye|vote nay|vote no)\b",
            low,
        ):
            first = re.split(r"[.!\n]", speech, maxsplit=1)[0].strip()
            if len(first) >= 24:
                exec_fx.append({"type": "set_goal", "id": "speech-goal", "text": first[:800]})
        if filed and (
            filed.group(1)
            or re.search(r"\b(workspace|design-log|artifact|charter|minute)\b", low)
        ):
            path = filed.group(1) or "design-log.md"
            exec_fx.append(
                {"type": "write_workspace", "path": path, "content": speech[:2000]}
            )
        if exec_fx:
            act["executive"] = exec_fx
